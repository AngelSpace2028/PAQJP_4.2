...
def load_pi_digits(filename: str = PI_DIGITS_FILE, expected_count: int = 100) -> Optional[List[int]]:
...
# Generate or load 100 base-10 digits of pi and map to base-256
def generate_pi_digits(num_digits: int = 100, filename: str = PI_DIGITS_FILE) -> List[int]:
    ...
    try:
        ...
        fallback_digits = [3, 1, 4, 1, 5, 9, 2, 6, 5, 3, 5, 8, 9, 7, 9, 3, 2, 3, 8, 4]
        mapped_fallback = [(d * 255 // 9) % 256 for d in fallback_digits]
        mapped_fallback = (mapped_fallback * (num_digits // len(mapped_fallback) + 1))[:num_digits]
        logging.warning(f"Using {len(mapped_fallback)} repeated fallback base-10 digits")
...
class SmartCompressor:
    ...
    def transform_07(self, data):
        """
        Optimized transformation using 100 base-10 digits of pi, mapped to 0-255.
    ...
    def transform_08(self, data):
        """
        New transformation using 100 base-10 digits of pi with circular shift and prime-based pre-transformation.
        """
        transformed = bytearray(data)
        pi_length = len(self.PI_DIGITS)
        data_size_kb = len(data) / 1024
    ...
    def compress_with_best_method(self, data, filetype):
        dt = datetime.now()
        ...
        if filetype in [Filetype.JPEG, Filetype.TEXT]:
            transformations = [(7, self.transform_07), (8, self.transform_08)] + transformations
    ...
    def decompress_with_best_method(self, data):
        ...
        try:
            ...
            return reverse_transforms[method_marker](decompressed), method_marker
        except Exception as e:
...
def main():
    print("PAQJP_4 Compression System with Base-10 Pi Transformation (100 digits)")
    ...
    if choice == '1':
        ...
        compressed = compressor.compress_with_best_method(input_data, filetype)
        if compressed is None:
            return
        ...
        try:
            ...
            # Preserve original extension based on marker (assuming 7 or 8 for .jpg/.txt)
            orig_ext = os.path.splitext(input_file)[1].lower()
            if marker in [7, 8] and not output_file.endswith(('.jpg', '.jpeg', '.txt')):
